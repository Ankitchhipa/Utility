...
public class SingleFingerView3 extends LinearLayout {
    private ImageView mView;
    private ImageView mPushView;
    private ImageView delete_view_2;
    private float _1dp;
    ...
    public SingleFingerView3(Context context) {
        this(context, null, 0);
    }

    public SingleFingerView3(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    @SuppressLint("ClickableViewAccessibility")
    public SingleFingerView3(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        this._1dp = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1, context.getResources().getDisplayMetrics());
        this.parseAttr(context, attrs);
        View mRoot = View.inflate(context, R.layout.item_view_sign_3, null);
        addView(mRoot, -1, -1);
        mPushView = mRoot.findViewById(R.id.push_view_3);
        mView = mRoot.findViewById(R.id.view_3);
    ...
    private void setViewToAttr(int pWidth, int pHeight) {
        ...
        int left = 0, top = 0;
        if (mCenterInParent) {
            left = pWidth / 4 - viewLP.width / 2;
            top = pHeight / 3 - viewLP.height / 2;
        } else {
